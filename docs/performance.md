...
|---|---|---|---|
| `bytes[]`, 500 × 256 B | 2126 µs | 390 µs | 5.5× |
| `bytes[]`, 2000 × 256 B | 8341 µs | 1563 µs | 5.3× |
| `uint256[]`, 1000 full-width words | 959 µs | 512 µs | 1.9× |
| nested tuples, depth 50 | 1684 µs | 45 µs | 37× |
| nested tuples, depth 200 | 24886 µs | 177 µs | 141× |
...
|---|---|---|---|
| `bytes[]`, 500 × 256 B | 382 µs | 19 µs | 20× |
| `bytes[]`, 2000 × 256 B | 1563 µs | 83 µs | 19× |
| `bytes[]`, 2000 × 100 B (unaligned) | 840 µs | 88 µs | 9.5× |
...
|---|---|---|---|
| `(bool × 8)` | **173 ns** | 177 ns | 1.02× slower |
| `(address, uint256)` — one call's arguments | **57 ns** | 107 ns | 1.88× slower |
| `(address, uint256, bytes)` | **195 ns** | 209 ns | 1.07× slower |
| `bool[]`, 100 elements | **1.59 µs** | 2.49 µs | 1.57× slower |
| `uint256[]`, 100 full-width words | **1.56 µs** | 3.80 µs | 2.44× slower |
| `(uint256, bool)[]`, 100 elements | **3.18 µs** | 8.00 µs | 2.52× slower |
| `bytes[]`, 100 × 256 B | **3.69 µs** | 11.04 µs | 2.99× slower |

**Compilation no longer pays on the encode side — every row is slower.** It
removes the *layout* overhead — the `Ty` match per value, the `isStatic` test
per component, the `List Part` a tuple or array allocates — which is what the
...
`bytes[]`, nil on `(bool × 8)`, one flat head section either way.

Decoding is untouched by that work and still mostly pays:

| shape | `decodeStrict` | compiled | |
|---|---|---|---|
| `(bool × 8)` | 569 ns | **276 ns** | 2.06× faster |
| `(uint256, bool)[]`, 100 elements | 21.96 µs | **11.17 µs** | 1.97× faster |
| `(address, uint256)` | 171 ns | **99 ns** | 1.73× faster |
| `bool[]`, 100 elements | 7.38 µs | **4.92 µs** | 1.50× faster |
| `bytes[]`, 100 × 256 B | 10.36 µs | **7.90 µs** | 1.31× faster |
| `uint256[]`, 100 full-width words | **3.04 µs** | 5.47 µs | 1.80× slower |
...
| read 32 big-endian bytes | 788 ns | 23 ns |
| read into limbs, `ba[i]!` | 17 ns | 17 ns |
